...
export const DailyTasks:FC<DailyTasksProps> = ({variant, list, setInput}) => {
    ...
    const closeAchieveFn = async (id:number) => {
        const res = await closeTask(token, id)
        setTaskRes(res)
        setTimeout(() => {
            setLoading({loading: false, taskId: id})
        }, 500)
        if(res?.errorData?.non_field_errors[0].length) {
            toaster(res?.errorData?.non_field_errors[0])
        } else {
            setTasks(id, res)
        }           
    }
    ...
    useEffect(() => {
        if(taskRes?.total_points) {
            setBalance(taskRes?.total_points, false)
        }
    }, [taskRes])

    const claimHandler = (link:string, id:number, category:string) => {
        if(category === 'link') {
                                    ...
                                    <>
                                        {
                                            loading?.loading && loading?.taskId === item.id ? (
                                                ...
                                                <>
                                                    Claim
                                                    <svg width="27px" height="15px" viewBox="0 0 66 43" version="1.1" xmlns="http://www.w3.org/2000/svg" >
